...
exports.router.get('/student', async (req, res) => {
    if (Object.keys(req.query).length == 0) {
        res.status(400)
            .send({ message: "Enter a query" });
    }
    try {
        const result = await db_1.pool.query("Select * FROM student_addmission where enroll_number=$1 OR first_name = $2 OR last_name= $3 OR fathers_name = $4 OR mothers_name = $5;", [req.query.enroll, req.query.firstname, req.query.lastname, req.query.fathersname, req.query.mothersname]);
...
exports.router.get('/teacher', async (req, res) => {
    if (Object.keys(req.query).length == 0) {
        res.status(400)
            .send({ message: "Enter a query" });
    }
    try {
        const result = await db_1.pool.query("SELECT * FROM teacher WHERE  first_name = $1 OR email = $2 OR phone = $3 OR emp_id = $4;", [req.query.name, req.query.email, req.query.phone, req.query.id]);
        res.send(result.rows);
    }
    catch (err) {
        console.log(err);
...
exports.router.get('/find', async (req, res) => {
    if (Object.keys(req.query).length == 0) {
        res.status(400)
            .send({ message: "Enter a query" });
    }
...
exports.router.post('/teacher/subject', async (req, res) => {
    try {
        const result = await db_1.pool.query("INSERT INTO public.teacher_grade(emp_id, subject_code, grade) VALUES($1, $2 ,$3)", [req.body.emp_id, req.body.subject_code, req.body.grade]);
        res.send("Created");
    }
    catch (err) {
        console.log(err);
...
exports.router.post('/student/create', async (req, res) => {
    try {
...
//     }
// })
exports.router.post('/student/addmission', async (req, res) => {
    try {
        console.log(req.body);
...
exports.router.post('/teacher/create', async (req, res) => {
    try {
        const result = await db_1.pool.query("INSERT INTO public.teacher(emp_id, first_name, last_name, gender, phone, email, address, primary_subject) VALUES($1, $2, $3, $4, $5, $6, $7, $8)", [req.body.emp_id, req.body.first_name, req.body.last_name, req.body.gender, req.body.phone, req.body.address, req.body.email, req.body.primary_subject]);
        res.send("Created");
    }
    catch (err) {
        console.log(err);
...
exports.router.post('/teacher/bulkcreate', async (req, res) => {
    try {
        console.log(req.body);
...
exports.router.put('/update/session', async (req, res) => {
    if (Object.keys(req.query).length == 0) {
        res.status(400)
            .send({ message: "Enter a query" });
    }
    try {
        const result = await db_1.pool.query("UPDATE public.student SET current_grade = $1, current_session = $2 WHERE enroll_number = $3", [req.body.current_grade, req.body.current_session, req.query.enroll]);
        res.send("Updated");
    }
    catch (err) {
        console.log(err);
